...
if __name__ == '__main__':
    spark = SparkSession.builder \
        .appName("examples") \
        .master('spark://a4.wanglab.ml:7077') \
        .config('spark.driver.memory', '2g') \
        .config('spark.executor.memory', '4g') \
        .config('spark.blockManager.port', '51814') \
        .config('spark.driver.port', '51813') \
        .config('spark.akka.frameSize', '128') \
        .getOrCreate()

    # Read in mnist_train.csv dataset
    df = spark.read.option("inferSchema", "true").csv('hdfs://a4.wanglab.ml:9000/csc2222/mnist_train.csv').orderBy(rand()).repartition(2)
